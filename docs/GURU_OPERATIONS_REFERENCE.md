# GURU — справочник операций (TRANSFER и INCOME)

**Назначение:** каноническое описание доменов **перевода** и **поступления** в том виде, как они реализованы в коде. Используйте для поддержки, регрессии и согласования изменений.

**Связанные документы:** `docs/GURU_PROJECT_CONTEXT.md`, `docs/GURU_ARCHITECTURE_AND_STANDARDS.md`, `docs/GURU_FULL_PROJECT_BLUEPRINT.md`.

**Якоря:** раздел [TRANSFER](#transfer) · раздел [INCOME](#income)
...
## TRANSFER (перевод)
**Назначение (TRANSFER):** единая точка правды по домену перевода **в том виде, как он реализован в коде сейчас**.
