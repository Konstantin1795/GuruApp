...
Проекты
Контрагенты
Аналитика за квартал (или за выбранный месяц по клику на столбец; повторный клик снимает выбор)
Доход / Задолженность / Переплата — из GET .../dashboard/analytics (0 ₽ при отсутствии данных)
Активные проекты (график по месяцам квартала) — из того же API; заголовок «Активные проекты» ведёт к списку проектов
Клики по суммам — bottom sheet со списком операций (GET .../dashboard/analytics/operations) и переходом к ReportDetail / TransferDetail; для строки агрегата переплаты по проекту — второй bottom sheet с GET .../dashboard/analytics/overpayment-detail (сводка + списки переводов и отчётов, дальше в детали операции)
История операций
Документы (плитка) → bottom sheet: Прайс-листы (ТЗ-10B) + пункт «Документы»
```
