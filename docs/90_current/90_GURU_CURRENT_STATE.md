...
**Обновлено:** 2026-05-13. Короткий снимок; детали — в `docs/README_MODULAR.md` и файлах `00_core` … `30_flutter`. Монолиты в `docs/OldDocs/legacy_monolith/` не канон.
...
- **Unified operations:** `GET …/operations/history` с параметром **`tab`** (`pending` | `all`): вкладка «На подтверждение» / «Все операции»; на клиенте **`AggregatedOperationsHistoryScreen`** (TabBar + TabBarView); **`combinedOperationsPendingCountProvider`** = сумма pending-count TRANSFER + INCOME и совпадает с вкладкой **pending** (без `WAITING_24_HOURS`, без «только откат», без **INCOME** `reset_approval` на этапе заказчика — тогда «на подтверждение» только у заказчика; для TRANSFER учтён **`complete_immediate`** у инициатора РП/Партнёра в **CREATED**). Для **OWNER** компании вкладка **all** — все операции компании; для **PARTNER / CUSTOMER** — только участие в строке операции.
...
- **ТЗ-12.1:** создание проекта вынесено в `CreateProjectService` — при добавлении полей создания проекта обновлять сервис и оба feature-теста (OWNER / PARTNER). Интеграционные smoke «TRANSFER/INCOME + дельты» — без изменений, перенос на ТЗ-10C+.
